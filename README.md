...
This project develops an anomaly detection system for the MetroPT3 air compressor dataset using a one-class autoencoder framework. The model is trained and evaluated on sequential windowed time series data, preserving the original class distribution **without additional rebalancing**. The system learns normal operating behavior and identifies deviations as potential failures, achieving a strong F1 score of 0.888 under realistic evaluation settings.
